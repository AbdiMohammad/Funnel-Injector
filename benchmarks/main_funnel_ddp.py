...
import engine.utils.imagenet_utils
import numpy as np
            ...
            if best_acc < acc:
                ...
                if args.mode == "prune":
                    ...
                    if save_state_dict_only:
                        torch.save(model_without_ddp.state_dict(), save_as)
                    else:
                        torch.save(model_without_ddp, save_as)
...
def main():
    ...
    engine.utils.imagenet_utils.utils.init_distributed_mode(args)
    ...
    if args.distributed:
        ...
        test_sampler = torch.utils.data.distributed.DistributedSampler(val_dst)
        ...
        test_loader = torch.utils.data.DataLoader(
            val_dst, batch_size=args.batch_size, sampler=test_sampler, num_workers=args.workers
        )
    else:
        train_loader = torch.utils.data.DataLoader(
            train_dst,
