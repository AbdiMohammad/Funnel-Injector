...
def main():
    ...
    if args.mode == "pretrain":
        ...
        # ch_sparsity_dict = {}
        # ch_sparsity_dict[model.get_submodule("layer1")] = 1
        ...
        for name, module in model.named_modules():
            if isinstance(module, torch.nn.Conv2d) and "shortcut" not in name:
                print(name + ":\t" + str(model.get_parameter(name + ".weight").data.shape[0]) + "/" + str(ori_model.get_parameter(name + ".weight").data.shape[0]) + "=" + str(model.get_parameter(name + ".weight").data.shape[0] / ori_model.get_parameter(name + ".weight").data.shape[0]))
                pruning_rates.append(model.get_parameter(name + ".weight").data.shape[0] / ori_model.get_parameter(name + ".weight").data.shape[0])
        print(pruning_rates)
        print(np.diff(np.array(pruning_rates)))
